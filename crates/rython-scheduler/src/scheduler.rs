use crate::{
    ...
    group::GroupState,
    task::{
        BgComplete, GroupCallback, ParallelTask, RecurringTask, RemoteTask, SequentialTask,
        TaskCallback, TaskOutput,
    },
};
use rython_core::{
...
pub struct TaskScheduler {
    ...
    ready_group_callbacks: Vec<(GroupId, GroupCallback, Vec<TaskOutput>)>,
...
impl TaskScheduler {
    ...
    pub fn submit_background_raw(
        &mut self,
        f: Box<dyn FnOnce() -> TaskOutput + Send + 'static>,
    ...
    {
        let erased_f =
            Box::new(move || -> TaskOutput { Ok(Box::new(f()) as Box<dyn Any + Send + 'static>) });

        let erased_cb: Option<TaskCallback> = callback.map(|cb| {
            Box::new(move |res: TaskOutput| {
        ...
        R: Any + Send + 'static,
    {
        ...
        self.pool.spawn(move || {
            let result: TaskOutput = Ok(Box::new(f()) as Box<dyn Any + Send + 'static>);
