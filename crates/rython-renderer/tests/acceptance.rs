...
fn qw3_2_scene_settings_clear_color_roundtrip() {
    let s = SceneSettings {
        clear_color: [0.05, 0.06, 0.14, 1.0],
        ..Default::default()
    };
...
fn t_rend_15_zero_commands_after_nonempty_frame() {
    ...
    assert_eq!(
        cmds.len(),
        0,
        "second drain after empty frame must return 0 commands"
    );
}
...
fn t_rend_17_mixed_command_variants_sorting() {
    ...
    assert_eq!(
        zs,
        vec![1.0, 2.0, 3.0, 4.0],
        "mixed variants must be sorted by z ascending"
    );
}
...
fn t_rend_19_camera_default_values() {
    ...
    assert!(
        cam.fov_degrees > 0.0,
        "default FOV must be positive, got {}",
        cam.fov_degrees
    );
...
fn t_rend_21_wgsl_validation_valid_shader() {
    ...
    assert!(
        result.is_ok(),
        "minimal valid WGSL shader must pass validation: {:?}",
        result.err()
    );
}
