...
fn t_thr_12_gil_not_held_during_rendering() {
    ...
    const _: () = {
        assert!(priorities::RENDER_ENQUEUE > priorities::GAME_LATE);
        assert!(priorities::RENDER_EXECUTE > priorities::GAME_LATE);
        assert!(priorities::RENDER_EXECUTE > priorities::RENDER_ENQUEUE);
    };
}
...
fn t_thr_13_gil_batch_efficiency() {
    ...
    const _: () = assert!(priorities::GAME_LATE - priorities::GAME_UPDATE == 5);
    ...
    const _: () = assert!(priorities::RENDER_ENQUEUE > priorities::GAME_LATE);
}
